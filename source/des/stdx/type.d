...
struct ElemInfo
{
    ...
    pure @safe nothrow @nogc
    {
        /++ get ElemInfo from type
         ...
         + works with:
         + * single numeric
         + * static arrays of numeric
         +/
        // TODO: user types with member `T[N] data` or `T[H][W] data`
...
/+ no doc because bad doc comment parsing for union
   members of union sets as public variables of module
   (hmod commit 406ba02)
 +/
union Array(T)
{
...
auto getTypedArray(T,X)( size_t sz, X* addr ) pure nothrow
{
    static if( is( Unqual!(X) == X ) )
        return Array!T( ArrayData( sz, addr ) );
    else
        return const Array!T( const ArrayData( sz, addr ) );
}
