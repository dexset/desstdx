...
unittest
{
    ...
    static assert( !isComplex!(typeof(3i)) );
}
...
unittest
{
    static assert( !hasBasicMathOp!int ); // can't get int from expr(int * double)
...
unittest
{
    ...
    @clot int var1;
    @zlot("op") string var2;
    static assert(  hasAttrib!(clot,var1) );
    static assert(  hasAttrib!(2,var2) );
}
...
template isPseudoInterface(I,T, bool _assert=true, string FILE=__FILE__, int LINE=__LINE__ )
{
    ...
    enum isPseudoInterface = checkMembers!(I,T,__traits(allMembers,I))();
}
